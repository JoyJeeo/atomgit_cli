...
涵盖：认证失败 / 权限不足 / 仓库不存在 / 受限仓库 / 仓库已禁用 / 分支不存在 / 请求参数错误 / 请求超时 / 网络连接失败 / LFS 指针验证失败 / 远端提交已创建但未确认等情形。底层异常原文不会输出，以免泄露 token、Authorization 头、签名 URL 或提交 SHA。
...
- 如果遇到网络错误，工具会自动重试
- 上传大文件时会显示进度条
- **上传失败语义化提示**：CLI 会把异常归类为「认证失败 / 权限不足 / 仓库不存在 / 受限仓库 / 仓库已禁用 / 分支不存在 / 请求参数错误 / 请求超时 / 网络连接失败 / LFS 指针验证失败 / 远端提交已创建但未确认 / 未知错误」并附可执行建议；401 建议重新登录，403 建议检查仓库或命名空间权限；服务端已返回合法提交但原始 pointer 无法确认时，会提示先检查远端再决定是否重试，同时不输出提交 SHA
