...
在全部下载成功后才执行。删除通过目录文件描述符逐层打开并拒绝跟随符号链接，
只 unlink 普通文件，不扫描目标树、不删除目录或未登记内容。缺失文件视为已清理；
manifest 无效、受管理路径变成目录/符号链接、下载或删除失败时命令失败且不推进
manifest。空仓库仍会进入该流程，因此可安全移除全部历史受管理文件。
