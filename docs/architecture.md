...
序列化包装在非 AtomGit 上下文中保持关闭。每个未确认 pointer 最多在同一 commit
执行三次只读验证并等待 2 秒、4 秒；该重试不重复上传、preupload 或 create-commit，
...
退避等待受 60 秒上限约束；请求超时不再生成上传总 deadline。作用域补丁在成功和失败后均恢复。
...
同一慢流协调器还产生匿名、完整覆盖的 Flow 状态：5 秒显示速度不进入恢复判断，
30 秒正式速度单独进入最多 12 项的趋势。隔离子进程通过独立的 256 条有界队列发送
最多 16 KiB 的 v1 JSON bytes；父进程约每 100 ms 排空并严格校验字段、范围和序号，
