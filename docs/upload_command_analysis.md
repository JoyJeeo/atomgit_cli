...
> 本文基于 `yuto` 分支版本 1.1.1 的当前源码。它同时记录已实现参数和已知缺陷，
...
该业务类型会显式传入各上传边界：只在原始类型为 `dataset` 时，精确过滤
HF 1.1.7 对 `.arrow` / `.parquet` 使用 model 传输路由的已知误报警。未指定类型或
显式 `model` 的同类提醒、以及 dataset 上传中的其他警告保持可见。
...
resumable 当前批次失败后，CLI 会从现有 HF 断点元数据补充显示已哈希、LFS 已
预上传和本地待确认的文件数与字节数；此前成功批次仍由累计确认汇总表示。
“LFS 已预上传”不代表远端提交已经确认，“本地待确认”也不代表远端一定没有提交。
单个元数据无法读取时显示为状态未知，不按零进度处理，也不会覆盖原始上传错误。
