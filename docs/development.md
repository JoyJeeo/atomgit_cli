...
需要修改 `cli/__init__.py`、`api/__init__.py` 和对应测试；禁止的是无关重构，而不是跨越真实
调用边界。
...
- 对外行为变化需要同步用户文档和示例。
- 每个新增或调整的产品特性必须在 `docs/features/` 下维护独立特性说明，
  明确标注当前状态，区分已实现行为与设计行为，并记录入口、边界、兼容性和
  验收重点；目录规则见 [features/README.md](features/README.md)。
- 内部重构不要求机械修改所有文档。
- 不创建只含注释、没有立即用途的占位源码。
