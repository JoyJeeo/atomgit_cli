...
网络问题失败。无需修改代码的稳妥方案是按自然目录或 20--30 个实际上传文件
划分批次，依次把每批内容上传到同一个 `--path-in-repo`。失败时只重试当前批次；
不要同时运行多个大目录上传进程争抢磁盘、网络和同一仓库分支。

CLI 在应用 `--ignore` 前打印原目录的文件数量和大小，所以该数字不表示最终上传
数量。双引号已经阻止 shell 展开通配符，不要写成 `\*\*`；递归忽略隐藏内容、
JSON 和 `output` 前缀内容应写为：
