...
代码提供 `--resumable`，但当前已验证存在与锁定 HF Hub 版本的参数兼容问题：
`upload_large_folder()` 不接受方法调用时传入的 `token`。在修复并完成远程中断/
恢复测试之前，应视为已知缺陷，而不是可靠能力。
...
代码提供相应参数，但已有远程报告使用了非法占位 repo ID，因此没有形成有效
测试。需要合法、唯一的 dataset 测试仓库重新验证。
