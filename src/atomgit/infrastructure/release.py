    ...
    if content_length:
        try:
            ...
            raise ReleaseValidationError(
                "release response has an invalid size"
            ) from error
        if too_large:
...
class _BoundedRedirectHandler(urllib.request.HTTPRedirectHandler):
    ...
    def redirect_request(self, req, fp, code, msg, headers, newurl):
        if not _allowed_origin(newurl, self.allowed_origins):
            raise ReleaseValidationError(
                "release download redirected to a disallowed origin"
            )
        return super().redirect_request(req, fp, code, msg, headers, newurl)


def fetch_bytes(
    url, *, limit=MAX_RESPONSE_BYTES, timeout=REQUEST_TIMEOUT, allowed_origins=None
):
    parsed = urllib.parse.urlparse(url)
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "atomgit-cli-installer",
        },
    )
    try:
        with opener.open(request, timeout=timeout) as response:
            ...
            if not _allowed_origin(final_url, allowed_origins):
                raise ReleaseValidationError(
                    "release response ended at a disallowed origin"
                )
...
def _release_assets(release, version, download_base=None):
    ...
    if (
        len(matches.get(wheel_name, [])) != 1
        or len(matches.get(checksum_name, [])) != 1
    ):
        raise ReleaseValidationError(
            "Release must contain exactly one expected wheel and checksum asset"
        )
    base = download_base or os.environ.get(
        "ATOMGIT_RELEASE_DOWNLOAD_BASE", DEFAULT_DOWNLOAD_BASE
    )
    urls = {}
    for name in (wheel_name, checksum_name):
        urls[name] = (
            matches[name][0].get("browser_download_url")
            or f"{base.rstrip('/')}/{version}/{name}"
        )
...
def validate_checksum(checksum_text, wheel_name):
    if (
        not isinstance(checksum_text, str)
        or len(checksum_text.encode("utf-8")) > MAX_CHECKSUM_BYTES
    ):
...
def validate_wheel_metadata(wheel_path, expected_version):
    ...
    if version_tuple(metadata.get("Version", "")) != expected_version:
        raise ReleaseValidationError(
            "wheel metadata version does not match the Release"
        )
    return True
...
def validate_release_assets(wheel_path, checksum_text, expected_version):
    ...
    if wheel_name != expected_name:
        raise ReleaseValidationError(
            "wheel filename does not match the Release version"
        )
...
def verify_installation(python, expected_version):
    ...
    if actual != expected_version:
        raise ReleaseError(
            f"installation reported {actual}, expected {expected_version}"
        )
    checks = [
    ...
    for command in checks:
        ...
        if result.returncode:
            raise ReleaseError(
                f"post-install verification failed: {' '.join(command[1:])}"
            )
    return actual
...
def install_release(
    ...
    *,
    ...
    download_origin = urllib.parse.urlparse(
        download_base
        or os.environ.get("ATOMGIT_RELEASE_DOWNLOAD_BASE", DEFAULT_DOWNLOAD_BASE)
    ).netloc.lower()
    with tempfile.TemporaryDirectory(prefix="atomgit-release-") as directory:
        ...
        validate_release_assets(
            wheel_path, checksum_path.read_text(encoding="utf-8"), target_version
        )
    ...
    package_location = subprocess.run(
        [
            python,
            "-c",
            "import importlib.metadata; print(importlib.metadata.distribution('atomgit').locate_file(''))",
        ],
        check=False,
        capture_output=True,
        text=True,
    ).stdout.strip()
    candidate_command = Path(scripts) / (
        "atomgit.exe" if os.name == "nt" else "atomgit"
    )
    command_path = (
        str(candidate_command) if candidate_command.exists() else "not visible on PATH"
    )
    return {
        "status": "installed",
...
def run_update(
    *,
    ...
    if (
        version is None
        and installed
        and version_tuple(target) < version_tuple(installed)
    ):
        raise ReleaseError(
            "latest completed Release is older than the installed version; use --version to downgrade"
        )
...
def _main(argv=None):
    ...
    result = install_release(
        args.version, python=args.python, force_reinstall=args.force_reinstall
    )
