...
def update(target_version, force_reinstall):
    ...
    return _lifecycle_commands.update(_COMMAND_CONTEXT, target_version, force_reinstall)
...
def delete_repository(repo_id, confirm):
    """永久删除仓库并验证远端已不存在"""
    return _repositories_commands.delete_repository(_COMMAND_CONTEXT, repo_id, confirm)
...
@click.option(
    "--verify-checksum", is_flag=True, help="使用仓库 checksum 校验已有文件和下载内容"
)
@click.option(
    "--resume",
    "resume_download",
...
@click.option("--prune", is_flag=True, help="清理 manifest 记录且远端已删除的本地文件")
@click.option(
    "--repo-type",
    "-r",
    "repo_type",
